...
test("user input incorrect value & show error message", async () => {
  ...
  expect(emailInput.value).not.toMatch(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/);
...
test("re-input form & error message dissapear", async () => {
  ...
  expect(fullnameInput.value).toMatch(/^[A-Za-z ]*$/);
  expect(emailInput.value).toMatch(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/);
  expect(phoneNumberInput.value).toMatch(/^((628|08)[\d]{9,11})$/);
  ...
  expect(screen.queryByText("No Handphone Tidak Sesuai")).not.toBeInTheDocument();
});
...
test('Select option program coding', ()=>{
  ...
  expect(screen.getByRole('option',{name:'Coding Backend with Golang'}).selected).toBe(true)
})
...
test('Upload surat kesungguhan', async()=>{
  ...
  const file = new File(["surat-kesungguhan"], "surat-kesungguhan.pdf", { type: "application/pdf"});
...
test('Reset button Click,', async ()=>{
  const { fullnameInput, emailInput,  phoneNumberInput, resetButton, radioIT, radioNonIT ,selectOptionProgram, inputFileKesungguhan, textArea , container} = setup();
  const jsdomAlert = window.alert; 
  window.alert = ()=>{};
